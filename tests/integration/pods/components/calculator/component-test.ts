...
module('Integration | Component | calculator', function(hooks) {
  ...
  test('should start at 0', async function(assert) {
    await render(hbs`<Calculator class="content-wrapper"/>`);
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '0');
  });
  ...
  test('should show entry on the display', async function(assert) {
    ...
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '5');

    await click('[data-value="*"]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '*');
  ...
  test('should append entries to display', async function(assert) {
    ...
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '56.8');
  });
  ...
  test('should only allow 12 digits to be entered', async function(assert) {
    ...
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '333333333333');
  });
  ...
  test('should clear', async function(assert) {
    ...
    await click('[data-value="3"]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '123');
  ...
  test('should negate number', async function(assert) {
    ...
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '-9');

    //other numbers
    await click('[data-value="AC"]');
    await click('[data-value="1"]');
    await click('[data-value="+-"]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '-1');
  });
  ...
  test('should multiply', async function(assert) {
    ...
    await click('[data-value="2"]');
    await click('[data-value="="]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '2');
    ...
    await click('[data-value="+-"]');
    await click('[data-value="="]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '4');
  ...
  test('should divide', async function(assert) {
    ...
    await click('[data-value="0"]');
    await click('[data-value="="]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), 'Infinity');
  });
  ...
  test('should calculate up to 12 digits', async function(assert) {
    ...
    await click('[data-value="2"]');
    await click('[data-value="="]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '6.444444444');
  });
  ...
  test('should format to 6 digits if length of the calculated answer is greater than 12', async function(assert) {
    ...
    await click('[data-value="9"]');
    await click('[data-value="="]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '1.00000e+14');
  });
  ...
  test('should calculate percent', async function(assert) {
    ...
    await click('[data-value="9"]');
    await click('[data-value="%"]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '0.09');
  });
  ...
  test('should continue calculating with answer', async function(assert) {
    ...
    await click('[data-value="5"]');
    await click('[data-value="="]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '10');
  });
  ...
  test('should calculate using order of operations', async function(assert) {
    ...
    await click('[data-value="5"]');
    await click('[data-value="="]');
    assert.equal(this.element.querySelector('.total')?.textContent?.trim(), '6');
  });
